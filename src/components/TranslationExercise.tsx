...
export default function TranslationExerciseA1(): JSX.Element {
  ...
  // cast `level` to any to satisfy getTranslationItems signature when level can be various types
  const items = useMemo<TranslationItem[]>(() => getTranslationItems(level as any) as TranslationItem[], [level]);
  const manualItems = useMemo<TranslationItem[]>(
    () => (getTranslationItems('A1') as TranslationItem[]),
    []
  ); // manual fallback for A1
  ...
  // debug: espone pool e questions globalmente e logga i conteggi in console
  (globalThis as any).__debug_pool = pool;
  (globalThis as any).__debug_questions = questions;
  console.debug('TranslationExercise debug — pool.length=', pool.length, 'questions.length=', questions.length, 'mode=', mode, 'subcategory=', subcategory, 'level=', level);
  ...
  const q = questions[index] as TranslationItem | undefined;
  // Guard: if no question available, show placeholder
  if (!q) {
    return (
      <div className="card">
        <div style={{ padding: 16 }}>
          <div>Nessuna domanda disponibile per la categoria selezionata.</div>
          <div style={{ marginTop: 8 }}>
            <button onClick={() => { setIndex(0); setValue(''); setCorrectCount(0); setChecked(null); setRevealed(false); }}>
              Reset
            </button>
          </div>
        </div>
    ...
    <div className="card">
      ...
      <div style={{ marginTop: 12, padding: 12, borderRadius: 8, border: '1px solid #e6e6e6', background: '#fff' }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <input
            ref={inputRef}
            placeholder={direction === 'EN→IT' ? 'Scrivi la traduzione in italiano' : 'Type the English translation'}
            value={value}
